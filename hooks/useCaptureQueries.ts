...
export const updateParsedEntry = (db: SQLiteDatabase, id: number, parsed: ParsedEntry) =>
  db.runAsync("UPDATE CaptureInbox SET parsed_json = ?, status = 'parsed' WHERE _id = ?",
    [JSON.stringify(parsed), id]);

// confirm returns the new AllTransactions rowid + keeps the raw text (for undo)
export const confirmInboxEntry = async (db: SQLiteDatabase, row: InboxRow, edited?: Partial<ParsedEntry>) => {
  const parsed: ParsedEntry = { ...JSON.parse(row.parsed_json ?? "{}"), ...edited };
  ...
  await db.runAsync("UPDATE CaptureInbox SET status = 'confirmed' WHERE _id = ?", [row._id]);
  return { txnId: res.lastInsertRowId as number, raw: row.raw_text, source: row.source, parsed };
};

// UNDO: delete the inserted txn, put the inbox row back as parsed
export const undoConfirm = async (db: SQLiteDatabase,
  undone: { txnId: number; raw: string; source: string; parsed: ParsedEntry }[]) => {
  await db.withTransactionAsync(async () => {
    for (const u of undone) {
      ...
      await db.runAsync(
        ...
        [u.raw, u.source, JSON.stringify(u.parsed), new Date().toISOString()]
      );
    }
  });
};