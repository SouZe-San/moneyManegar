...
const getTotalIncomeMonthWise = async (db: SQLiteDatabase) => {
  try {
    const rows = await db.getAllAsync<{ total: number }>(
...
const getTotalExpenseMonthWise = async (db: SQLiteDatabase) => {
  try {
    const rows = await db.getAllAsync<{ total: number }>(
      `SELECT SUM(amount) as total
       FROM AllTransactions
...
const clearBudgetTable = async (db: SQLiteDatabase) => {
  try {
...
const deleteBudget = async (db: SQLiteDatabase, budgetId: string) => {
  try {
